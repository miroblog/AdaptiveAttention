...
class AdaptiveAttention(nn.Module):
    def __init__(self):
        ...
        self.w_h = nn.Parameter(torch.randn(1, N_IMAGE_DESCRIPTOR))
        self.W_v = nn.Parameter(torch.randn(IMAGE_RAW_DIM,N_IMAGE_DESCRIPTOR))
        self.W_g = nn.Parameter(torch.randn(HIDDEN_DIM,N_IMAGE_DESCRIPTOR))

        self.W_s = nn.Parameter(torch.randn(N_IMAGE_DESCRIPTOR,HIDDEN_DIM))
    ...
    def forward(self, V, s_t, h_t):
        ...
        z_t = torch.matmul(F.tanh(image_part+hidden_part), self.w_h)
        ...
        # eq(12)
        attention_vs = torch.matmul(F.tanh(torch.matmul(s_t, self.W_s) + single_hidden_part), self.w_h)
        concatenates = torch.cat([z_t, attention_vs], dim=1)
        alpha_t_hat = F.softmax(concatenates, dim=1)

        # beta_t = alpha_t[k+1] , last element of the alpha_t
        beta_t = alpha_t_hat[-1]
        # eq(11)
        c_t_hat = beta_t.mul(s_t) + (1-beta_t).mul(c_t)
        return c_t_hat
...
class VisualSentinel(nn.Module):
    def __init__(self):
        ...
        self.W_x = nn.Parameter(torch.randn(IMAGE_RAW_DIM+EMB_DIM, HIDDEN_DIM))
        self.W_h = nn.Parameter(torch.randn(HIDDEN_DIM, HIDDEN_DIM))

    # m_t is the lstm cell state
    def forward(self, x_t, prev_h_t, m_t):
        # eq (9) / (10)
        g_t = F.sigmoid(torch.matmul(x_t, self.W_x) + torch.matmul(prev_h_t, self.W_h)) # output -1, 512, 512
        s_t = torch.mul(g_t, F.tanh(m_t))
        return s_t
